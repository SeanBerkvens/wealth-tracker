...
export default function AssetActions({ id, name, category, value, icon, iconColor }: AssetActionsProps) {
  ...
  return (
    <>
      <div className="relative">
        <button type="button" onClick={toggleMenu} title="Asset options" className="rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground btn-press"><EllipsisVertical className="h-6 w-6" /></button>
        {menuOpen && (
          <div className="option-menu absolute right-0 z-20 mt-1 w-28 rounded-lg border border-border bg-popover p-1 shadow-lg">
            ...
            <button type="button" onClick={deleteAsset} className="w-full rounded-md px-3 py-2 text-left text-sm text-red-500 hover:bg-red-500/10">Delete</button>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 modal-overlay">
          <div className="w-full max-w-md space-y-4 rounded-2xl border border-border bg-card p-6 modal-content">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button onClick={updateAsset} className="rounded-lg bg-primary px-4 py-2 text-primary-foreground btn-press">Save</button>
            </div>
          </div>
        </div>
      )}
